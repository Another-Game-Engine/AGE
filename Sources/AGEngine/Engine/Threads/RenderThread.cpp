...
namespace AGE
{
	...
	void RenderThread::getCube(Key<Vertices> &v, Key<Painter> &p)
	{
		...
		auto type = std::make_pair<GLenum, StringID>(GL_FLOAT_VEC3, StringID("position"));
		...
		vertices->set_data<glm::vec3>(positions, StringID("position"));
										...
										std::shared_ptr<Painter> &line3DPainterDepth)
	{
		...
		if (debug2DlinesPoints.size() != 0)
		{
			...
			auto type = std::make_pair<GLenum, StringID>(GL_FLOAT_VEC2, StringID("position"));
			std::vector<std::pair < GLenum, StringID > > types;
			types.push_back(type);
			type = std::make_pair<GLenum, StringID>(GL_FLOAT_VEC3, StringID("color"));
			types.push_back(type);
			...
			vertices->set_data<glm::vec2>(debug2DlinesPoints, StringID("position"));
			vertices->set_data<glm::vec3>(debug2DlinesColor, StringID("color"));
			vertices->set_indices(indices);
		...
		if (debug3DlinesPoints.size() != 0)
		{
			...
			vertices->set_data<glm::vec3>(debug3DlinesPoints, StringID("position"));
			vertices->set_data<glm::vec3>(debug3DlinesColor, StringID("color"));
			vertices->set_indices(indices);
		...
		if (debug3DlinesPointsDepth.size() != 0)
		{
			...
			vertices->set_data<glm::vec3>(debug3DlinesPointsDepth, StringID("position"));
			vertices->set_data<glm::vec3>(debug3DlinesColorDepth, StringID("color"));
			vertices->set_indices(indices);
	...
	void RenderThread::getQuadGeometry(Key<Vertices> &v, Key<Painter> &p)
	{
		...
		vertices->set_data<glm::vec2>(positions, StringID("position"));
		vertices->set_indices(indices);
