...
{ \
	return isArray == false ? std::static_pointer_cast<AProgramResources>(std::make_shared<Mat4>(glm::mat4(1.0f), id, std::move(name))) : std::static_pointer_cast<AProgramResources>(std::make_shared<Mat4Array255>(nullptr, id, std::move(name))); \
}), \
