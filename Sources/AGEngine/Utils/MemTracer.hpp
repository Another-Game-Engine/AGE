...
void recursiveImguiTreeDisplay(LiveMemTracer::Edge *edge, bool updateData)
{
	if (edge->count)
	{
		if (edge->count > 1024)
		...
		if (ImGui::TreeNode(edge->name))
		{
			...
			{
				recursiveImguiTreeDisplay(edge->to[i], updateData);
			}
			ImGui::TreePop();
		}
	}
...
void LiveMemTracer::display(float dt)
{
	...
	if (ImGui::Begin("LiveMemoryProfiler"))
	{
		...
		else
		{
			...
			{
				recursiveImguiTreeDisplay(e, updateData);
			}
		}
	}
	ImGui::End();
