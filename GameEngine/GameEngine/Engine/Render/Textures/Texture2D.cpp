...
namespace AGE
{
	...
	Texture2D &Texture2D::setCompressed(std::vector<uint8_t> const &data, GLint level, GLenum format, GLsizei imageSize)
	{
		glCompressedTexSubImage2D(GL_TEXTURE_2D, level, 0, 0, _width, _height, format, imageSize, data.data());
		return (*this);
	}
