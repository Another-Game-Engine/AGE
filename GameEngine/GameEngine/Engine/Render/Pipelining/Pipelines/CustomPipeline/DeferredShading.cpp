...
namespace AGE
{
		...
		ARenderingPipeline(std::string("deferred shading"), painter_manager)
	{
		...
		_programs[STENCIL_BASIC] = std::make_shared<Program>(Program(std::string("program_stencil"), { std::make_shared<UnitProg>(DEFERRED_SHADING_STENCIL_VERTEX, GL_VERTEX_SHADER), std::make_shared<UnitProg>(DEFERRED_SHADING_STENCIL_FRAG, GL_FRAGMENT_SHADER) }));
		...
		_lightAccumulationTexture = addRenderPassOutput<Texture2D, RenderingPass>(_rendering_list[LIGHTNING], GL_COLOR_ATTACHMENT0, screen_size.x, screen_size.y, GL_RGBA8, true);
	}
	...
	IRenderingPipeline & DeferredShading::render(RenderPipeline const &pipeline, RenderLightList const &lights, CameraInfos const &infos)
	{
		...
		*_programs[LIGHTNING]->get_resource<Sampler2D>("normal_buffer") = _normalTexture;
		*_programs[LIGHTNING]->get_resource<Sampler2D>("depth_buffer") = _depthTexture;

		for (auto &pl : lights.pointLight)
		{
			*_programs[LIGHTNING]->get_resource<Vec3>("position_light") = glm::vec3(pl.light.sphereTransform[3]);
			*_programs[LIGHTNING]->get_resource<Vec3>("attenuation_light") = pl.light.attenuation;
			*_programs[LIGHTNING]->get_resource<Vec3>("color_light") = pl.light.color;
			*_programs[LIGHTNING]->get_resource<Vec3>("ambiant_color") = glm::vec3(0);
