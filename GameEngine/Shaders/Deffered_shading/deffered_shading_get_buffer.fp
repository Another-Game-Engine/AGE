...
vec3 perturb_normal()
{
	...
	vec3 normal_bump_adjust = normalize(normal_bump * 2.f - vec3(1.f));
	return mix(normalize(transpose(TBN) * normal_bump_adjust), inter_normal, all(equal(normal_bump, vec3(0.f))));
}
