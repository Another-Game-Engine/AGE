...
#include <Scenes/SkyboxEditorScene.hh>
...
int			main(int ac, char **av)
{
	...
	engine->launch(std::function<bool()>([&]()
	{
		...
		engine->addScene(std::make_shared<AGE::SkyboxEditorScene>(engine), AGE::SkyboxEditorScene::Name);
		...
		if (!engine->initScene(AGE::WorldEditorScene::Name))
			return false;
		if (!engine->initScene(AGE::SkyboxEditorScene::Name))
			return false;
